...
model:add(loadcaffe.load(prototxt,caffemodel,'nn'):cuda())
model.modules[3].modules[24] = nn.View(10):cuda()
--model.modules[3]:add(nn.View(10):cuda())
for k,v in pairs(model.modules[3]:findModules('nn.SpatialConvolutionMM')) do
...
provider = torch.load 'provider01Norm.t7'
